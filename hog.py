...
def free_bacon(score):
	...
	# BEGIN PROBLEM 2
	replacement = 2 * (score // 10 % 10) - (score % 10)
	if replacement < 1:
		return 1
	return replacement
	# END PROBLEM 2
...
def is_swap(player_score, opponent_score):
	...
	"""
	# BEGIN PROBLEM 4
	player_difference = abs((player_score // 10)  % 10 - (player_score % 10))
	opponent_difference = abs((opponent_score // 10) % 10 - (opponent_score % 10))
...
def announce_highest(who, previous_high=0, previous_score=0):
	...
	# BEGIN PROBLEM 7
	syntax = "point(s)! That's the biggest gain yet for Player"
	def say(score0, score1):
		if who:
			if previous_high < (score1 - previous_score):
				print(score1 - previous_score, syntax, 1)
				return announce_highest(who, score1 - previous_score, score1)
			return announce_highest(who, previous_high, score1)
		else:
			if previous_high < (score0 - previous_score):
				print(score0 - previous_score, syntax, 0)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
	...
	temp, i = 0, 1
	for x in range(1, 11):
		if (average(x, dice) > temp and i <= x):
			temp, i = average(x, dice), x
	return i
	# END PROBLEM 9
